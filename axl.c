...
static const char *
json_parse_object (xml_t e, const char *json, int inarray)
{                               // parse a JSON object, return next char after object or NULL for error
   ...
   void addvalue (int a, int inarray)
   {
      ...
      {                         // object
         ...
         if (!inarray)
            o->json_single = 1;
         json = json_parse_object (o, json, inarray);
      } else if (*json == '[')
...
xml_tree_parse_json (const char *json, const char *rootname)
{
   ...
   if (*json == '{' || *json == '[')
      json = json_parse_object (t, json, 0);    // It will work out [ is an array
   else
