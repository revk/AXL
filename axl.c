...
static int
strcmp_l (int al, const char *a, int bl, const char *b)
{
   if (!al && !bl)
      return 0;
   if (!a || al < bl)
...
{                               // Parse null terminated XML data
   ...
   int nssp = 0,
      nssn = 0;
   void ns_stack (int tagl, const unsigned char *tag, int namespacel, char *namespace)
   {                            // Add to stack
      if (nssp + 1 > nssn)
...
#error CPP not coded
   ...
   inline unsigned int isnamestartchar (unsigned int c)
   {                            // https://www.w3.org/TR/xml/#NT-NameStartChar
      if (c == ':' || c == '_' || (c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z') || (c >= 0xC0 && c <= 0xD6)
          || (c >= 0xF8 && c <= 0x2FF) || (c >= 0x370 && c <= 0x37D) || (c >= 0x37F && c <= 0x1FFF) || (c >= 0x200C && c <= 0x200D)
          || (c >= 0x2070 && c <= 0x218F) || (c >= 0x2C00 && c <= 0x2FEF) || (c >= 0x3001 && c <= 0xD7FF) || (c >= 0xF900
                                                                                                              && c <= 0xFDCF)
          || (c >= 0xFDF0 && c <= 0xFFFD) || (c >= 0x10000 && c <= 0xEFFFF))
         return c;
      return 0;
   }
   inline unsigned int isnamechar (unsigned int c)
   {                            // https://www.w3.org/TR/xml/#NT-NameChar
      if (isnamestartchar (c) || c == '-' || c == '.' || (c >= '0' && c <= '9') || c == 0xB7 || (c >= 0x300 && c <= 0x36F)
          || (c >= 0x203F && c <= 0x2040))
         return c;
      return 0;
   }
   inline unsigned int isnames (unsigned int c)
   {                            // https://www.w3.org/TR/xml/#NT-Names
      if (isnamechar (c) || c == ' ')
         return c;
      return 0;
   }
   const unsigned char *parse_name (void)
   {                            // Parse name, NULL if not a valid name
...
xml_t
xml_element_add_ns_after_l (xml_t parent, xml_namespace_t namespace, int namel, const char *name, xml_t prev)
...
xml_attribute_t
xml_attribute_set_ns_l (xml_t e, xml_namespace_t namespace, int namel, const char *name, int contentl, const char *content)
{
   if (!name)
   ...
   xml_attribute_t a = e->first_attribute;
   while (a
          && ((a->namespace != namespace && (namespace || a->namespace != e->namespace))
              || (name && strcmp_l (strlen (a->name ? : ""), a->name, namel, name))))
      a = a->next;
   if (!content)
   {
      if (a)
...
cmp_attribute (const void *A, const void *B)
{
   xml_attribute_t a = *(xml_attribute_t *) A,
      b = *(xml_attribute_t *) B;
   int r = strcmp (a->namespace ? a->namespace->uri : "", b->namespace ? b->namespace->uri : "");
   if (r)
      return r;
...
#else
static xml_t
xml_tree_read_f (FILE * i, const char *file)
{
   char *xml = NULL;
   size_t len = 0;
   ...
   void er (const char *filename, int line, int character, const char *error, const unsigned char *posn)
   {
      fprintf (stderr, "XML parse error in %s line %d:%d (%s)", filename ? : "stdin", line, character, error);
      if (posn)
      {
         fprintf (stderr, " at: ");
         if (character > 51)
...
xml_pi_t
xml_pi_add_l (xml_t t, int namel, const char *name, int contentl, const char *content)
{
   if (!t)
...
void
xml_element_set_content_l (xml_t e, int contentl, const char *content)
{
   if (!e)
...
xml_t
xml_add_free (xml_t e, const char *path, char *value)
{
...
size_t
xml_based (const char *src, unsigned char **buf, const char *alphabet, unsigned int bits)
...
xml_log (int debug, const char *who, const char *what, xml_t tx, xml_t rx)
{
   ...
   p +=
      snprintf (p, path + sizeof (path) - p, "%06lu-%s-%s", (long) tv.tv_usec, norm (strdupa (who ? : "-")),
                norm (strdupa (what ? : "-")));
