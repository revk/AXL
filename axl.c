...
{
  xml_namespacelist_t a = *(xml_namespacelist_t *) A, b = *(xml_namespacelist_t *) B;
...
{
  xml_attribute_t a = *(xml_attribute_t *) A, b = *(xml_attribute_t *) B;
...
sort_attributes (xml_t e)
{
  ...
  xml_attribute_t *aa = &e->first_attribute, p = NULL;
  for (i = 0; i < c; i++)
    {
      l[i]->prev = p;
      *aa = l[i];
...
xml_add (xml_t e, const char *path, const char *value)
{
  ...
  while (*path)
    {
      char add = 0, att = 0, single = 0, unquoted = 0;
      while (*path == '/')
	path++;
      // Special prefixes
      for (; *path; path++)
...
{				// Post (if tree supplied) or Get a URL and collect response
  ...
  if (!curl)
    curl = curl_easy_init ();
  // response file
  char *reply = NULL, *request = NULL;
  size_t replylen = 0, requestlen = 0;
